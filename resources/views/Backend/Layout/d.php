...
<head>
  ...
  <title>MapleSalon | Dashboard</title>
...
<body>
  ...
  <div class="dashboard-container">
    <div class="sidebar">
      <div class="logo"><i class="fas fa-spa logo-icon"></i><h2>Maple Salon</h2></div>
      <div class="nav-menu">
        <a href="{{ url('/Admin') }}" class="nav-item {{request()->routeIs('dashboard') ? 'active' : ''}} animate-fadeIn delay-1"><i class="fas fa-home"></i><span>Dashboard</span></a>
       <a href="{{ route('list.booking') }}"
        ...
        <a href="{{ route('list.product') }}"
          ...
          <i class="fas fa-box"></i><span>Booking & Contact</span>
        </a>

        <a href="{{ route('list.service') }}" class="nav-item animate-fadeIn delay-3"><i class="fas fa-cut"></i><span>Services</span></a>
        <a href="{{ route('list.branch') }}" class="nav-item {{request()->routeIs('list.branch') ? 'active' : ''}} animate-fadeIn delay-4"><i class="fas fa-store"></i><span>Branch</span></a>
        <a href="{{ route('reports.index') }}" class="nav-item animate-fadeIn"><i class="fas fa-file-alt"></i><span>Report</span></a>
        <a href="{{ route('list.user') }}" class="nav-item animate-fadeIn"><i class="fas fa-user"></i><span>User</span></a>
        <a href="{{ route('login') }}" class="nav-item animate-fadeIn"><i class="fas fa-sign-out-alt"></i><span>Logout</span></a>
      </div>
    </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      if (mobileMenuToggle) {
        mobileMenuToggle.addEventListener('click', function () { body.classList.toggle('sidebar-open'); });
      }
      ...
      cancelButtons.forEach((btn) => {
        btn.addEventListener('click', function () {
          ...
          if (bookingId && confirm(`Are you sure you want to cancel booking ${bookingId}?`)) {
            ...
            if (statusSpan) { statusSpan.textContent = 'Cancelled'; statusSpan.className = 'status cancelled'; }
          }
        });
      });

      const statCards = document.querySelectorAll('.stat-card');
      statCards.forEach((card, index) => { card.style.animation = `fadeIn 0.5s ease ${index * 0.1}s forwards`; card.style.opacity = 0; });
    });
  </script>
</body>
</html>
