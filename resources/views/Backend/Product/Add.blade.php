...
<div class="main-content">
  ...
  <div class="card">
    ...
    <div class="card-body">
     <form action="{{ route('store.product') }}" method="POST" novalidate>
  @csrf

  <div class="form-grid">
    <div class="form-column">
      {{-- Customer (replaces Branch) --}}
      <div class="form-group">
        <label for="customer">Customer (name) <span class="required">*</span></label>
        <input id="customer" name="customer" type="text" class="form-control"
               value="{{ old('customer') }}" maxlength="120" placeholder="e.g., Sokha Phan" required>
    ...
    <div class="form-column">
      <div class="form-group">
        ...
        <input id="qty" name="qty" type="number" class="form-control"
               value="{{ old('qty', 1) }}" step="1" min="0" required>
        @error('qty') <span class="error-message">{{ $message }}</span> @enderror
      </div>

      {{-- Calculated total (hidden + readonly view) --}}
      <div class="form-group d-none">
        <label for="total_view">Total Price</label>
        <input id="total_view" type="text" class="form-control" value="{{ old('total','0.00') }}" readonly>
        <input id="total" name="total" type="hidden" value="{{ old('total','0.00') }}">
        @error('total') <span class="error-message">{{ $message }}</span> @enderror
      </div>

      <div class="form-group">
        <label for="memo">Memo</label>
        <textarea id="memo" name="memo" class="form-control" rows="1" maxlength="255" placeholder="Optional">{{ old('memo') }}</textarea>
  ...
  <div class="form-actions">
    <a href="{{ route('list.product') }}" class="btn btn-cancel"><i class="fas fa-times"></i> Cancel</a>
